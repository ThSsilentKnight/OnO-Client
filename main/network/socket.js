...
ws.addEventListener("message", async (event) => {
    ...
    switch (request.action) {
        ...
        case "send_client_room_info":
            // Code for this function is running in requests.ts/js | data gets pushed to session storage
            break;
...
function connectClientSetUp() {
    ...
    if (!data || !clientId || !roomId)
        return;
    const parsed = JSON.parse(data);
    requestRejoinRoom(clientId, roomId, parsed.color, parsed.smallRings, parsed.mediumRings, parsed.largeRings, sessionStorage.getItem("username"));
}
